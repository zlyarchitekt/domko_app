...
>
> **Naprawa 2026-07-02 (Claude):** Naprawiono zgłoszony przez użytkownika bug (obrys nie zamykał się przy rysowaniu klik-po-klik, patrz F1-04 — root cause: auto-centrowanie widoku reagujące na punkty rysowane na żywo, przez co mapowanie klik→współrzędna dryfowało między kliknięciami) oraz wszystkie 3 bugi znalezione w audycie wyżej: **F5-05/F5-06** (backend teraz wystawia pełny `layout` per wariant optymalizatora — `VariantModel` miał `id`/`layout` całkowicie pominięte mimo że dane były już policzone; naprawiono też błędny request z frontendu, który wysyłał pola nieistniejące w kontrakcie `/optimizer/run` zamiast wymaganych `latitude`/`longitude`, więc realnie kończył się 422 przed dotarciem do jakiegokolwiek renderu), **F6-05** (endpoint `/export/pdf` przepuszczał surowy request zamiast liczyć snapshot jak `/export/json` — naprawione + nowa funkcja `layout_result_to_response`/`_build_pdf_report_data` reużywająca istniejącą logikę). **Odkryty przy okazji, nieplanowany bug:** `UUID(data.get("project_id", uuid4()))` w `export_json.py`/`export_dxf.py` rzucał `AttributeError` przy każdym requeście bez jawnego `project_id` (czyli zawsze, z realnego frontendu) — **`/export/json` i `/export/dxf` (F6-03/F6-01) były więc też realnie zepsute mimo statusu „done"**, teraz naprawione. Wszystkie cztery naprawy zweryfikowane end-to-end w przeglądarce (Playwright, nie tylko `pytest`): rysowanie+zamknięcie obrysu → `/layout/generate` 200; optymalizator → 3 karty wariantów bez crashu → „Zastosuj układ” faktycznie zmienia canvas; JSON/DXF/PDF pobierają się poprawnie, PDF ma realne wartości (nie zera). Backend 96/96 testów (2 nowe w `test_export_pdf.py`), `ruff` czysty, `npm run build` czysty.
---
...
| `t_e5c6ec52` | **F1-04** | ✅ done | P1 | Rysowanie wielokąta klik-po-klik. (Zrealizowane przez Gemini). **[Naprawa, 2026-07-02]** Zgłoszony przez użytkownika bug: nie dało się zamknąć rysowanego obrysu. Root cause: `CanvasEditor.tsx` miał `useEffect` auto-centrujący widok (`setPosition(initialCentered)`) zależny od `dataBounds`, który z kolei zależał od `state.drawingPoints` — więc **widok recentrował się po każdym kliknięciu w trakcie rysowania**, przesuwając `position.x/y` używane przez `worldToMeters()`. Efekt: mapowanie klik-ekranu→współrzędna-świata dryfowało między kolejnymi kliknięciami, więc kliknięcie wizualnie „na" pierwszym punkcie nie trafiało w promień zamknięcia (1.5m) w (już nieaktualnym) układzie współrzędnych — obrys nigdy się nie zamykał, więc `/api/v1/layout/generate` nie miał na czym działać. Naprawione: auto-centrowanie reaguje teraz tylko na zatwierdzony `footprint`, nie na punkty rysowane na żywo (`footprintBounds`/`footprintCenter`, odseparowane od `dataBounds` używanego przez manualny przycisk „Fit to screen”). Zweryfikowane end-to-end w przeglądarce (Playwright): rysowanie 4 punktów + zamknięcie kliknięciem blisko pierwszego punktu daje poprawny kwadrat, `/layout/generate` zwraca 200. |
...
| `t_0138b5c4` | **F5-05** | ✅ done | — | **[Naprawa, 2026-07-02]** Root cause: backendowy `VariantModel` nigdy nie wystawiał pól `id`/`score`/pełnego `layout`, mimo że `OptimizerVariant.layout` (pełny `LayoutResult`) był liczony wewnętrznie i po prostu odrzucany w `_variant_to_model()`. Naprawione w `backend/api/v1/endpoints/optimizer.py`: dodano `id` (`variant-{rank}`) i `layout: LayoutGenerateResponse` (nowa współdzielona funkcja `layout_result_to_response()` w `layout.py`, reużywana przez `/layout/generate`). Frontend: `api.ts`'s `OptimizerVariant`/`OptimizerRunRequest` przepisane 1:1 pod realny kontrakt backendu (poprzednio wysyłał nieistniejące `circulation`/`num_generations`/`population_size` zamiast wymaganych `latitude`/`longitude` — **request zawsze kończyłby się 422**, więc karta wariantów nigdy realnie się nie renderowała w produkcji). `OptimizerSection.tsx` pokazuje realne `metrics.wt_compliance`/`metrics.solar_score` zamiast nieistniejącego `v.score`. Zweryfikowane end-to-end w przeglądarce (Playwright): 3 karty wariantów renderują się bez błędu konsoli/crasha. |
| `t_45e4f171` | **F5-06** | ✅ done | — | **[Naprawa, 2026-07-02]** Dodano `applyVariant(id)` w `SessionContext.tsx`, dispatchujące `SET_LAYOUT_RESULT` z `variant.layout` (patrz F5-05 — backend teraz to pole faktycznie wystawia). Nowy przycisk „Zastosuj ten układ” w `OptimizerSection.tsx`. Zweryfikowane end-to-end: po kliknięciu canvas i pasek boczny („Bilans”, powierzchnie mieszkań) faktycznie aktualizują się na dane wybranego wariantu. |
...
| `t_cfec375d` | **F6-03** | ✅ done | — | `/api/export/json` — zarejestrowany, działający, zwraca pełny snapshot. **[Naprawa, 2026-07-02]** Odkryty i naprawiony przy okazji F6-05: `services/export_json.py`'s `build_export_payload_from_request()` robił `UUID(data.get("project_id", uuid4()))` — gdy `project_id` brakuje w requeście (zawsze, bo frontend go nie wysyła), `.get()` zwraca już-gotowy obiekt `UUID`, który trafia z powrotem do konstruktora `UUID()` i rzuca `AttributeError`. Endpoint był więc realnie zepsuty (500) dla każdego prawdziwego wywołania z frontendu, mimo zielonych testów (żaden test nie wywoływał go bez jawnego `project_id`). Naprawione: `UUID(str(data["project_id"])) if data.get("project_id") else uuid4()`. Zweryfikowane end-to-end w przeglądarce — pobranie JSON działa. |
| `t_ceb5f54b` | **F6-04** | ✅ done | — | **[Gemini, Fala 4]** Przycisk [Eksport JSON] podłączony i działający w UI. Zweryfikowane end-to-end po naprawie F6-03. |
| `t_00cbe528` | **F6-05** | ✅ done | — | **[Naprawa, 2026-07-02]** Root cause: `/api/v1/export/pdf` przepuszczał surowy request wprost do `export_project_pdf()` zamiast (jak deklarował własny docstring — „mirrors the JSON export endpoint") najpierw policzyć snapshot jak `/export/json`. Naprawione w `api/v1/endpoints/export.py`: endpoint teraz woła `build_export_payload_from_request()` + `export_project_json()`, po czym nowa funkcja `_build_pdf_report_data()` spłaszcza wynik do kształtu, jakiego oczekuje `export_project_pdf()` (`score`, `footprint_area_m2`, `apartments[].min_width_m/passed` — liczone przez `services.apartment_validation.validate_apartment`-owy próg `MIN_ROOM_WIDTH_M`, `facades` — spłaszczone z `solar_analysis.apartments[].facades`). Ten sam `UUID(project_id)` bug co F6-03 naprawiony też w `export_dxf.py` (dotyczył też `/export/dxf`). `test_export_pdf.py` przepisany: stary test lockował błędne zachowanie (wysyłał już-gotowy płaski payload prosto do endpointu) — teraz testuje zarówno renderer (`export_project_pdf`) jak i endpoint z realnym, surowym payloadem projektu. Zweryfikowane end-to-end w przeglądarce: pobrany PDF ma realne wartości (np. wynik 100/100, powierzchnia obrysu 60.0 m² — nie zera). |
